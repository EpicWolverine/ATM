...
Public Class Form1
    ...
    Private Sub btnDeposit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDeposit.Click
        If txtDeposit.Text = "Deposited!" Or txtDeposit.Text = "" Or IsNumeric(txtDeposit.Text) = False Then
            MsgBox("Please enter an amount to deposit.")
            Exit Sub
        Else
            Dim s As System.Text.StringBuilder
            ...
            If rbBankDeposit.Checked = True Then 'if depositing in the bank
                ...
                Dim total As New System.IO.StreamWriter(ResourceFilePath & "\total.txt") 'write to total
                total.WriteLine(c.ToString)
                total.WriteLine(b.ToString)
                total.Close()

                Dim history As New System.IO.StreamWriter(ResourceFilePath & "\history.txt", True) 'write to history
                history.WriteLine("Bank: +$" & txtDeposit.Text)
                history.Close()
            ElseIf rbWalletDeposit.Checked = True Then 'if depositing in the wallet
                ...
                Dim total As New System.IO.StreamWriter(ResourceFilePath & "\total.txt") 'write to total
                total.WriteLine(a.ToString)
                total.WriteLine(c.ToString)
                total.Close()

                Dim history As New System.IO.StreamWriter(ResourceFilePath & "\history.txt", True) 'write to history
                history.WriteLine("Wallet: +$" & txtDeposit.Text)
                history.Close()
            End If
    ...
    Private Sub btnWithdrawal_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnWithdrawal.Click
        If txtWithdrawal.Text = "Withdrawed!" Or txtWithdrawal.Text = "" Or IsNumeric(txtWithdrawal.Text) = False Then
            MsgBox("Please enter an amout to withdraw.")
            Exit Sub
        Else
            Dim s As System.Text.StringBuilder
